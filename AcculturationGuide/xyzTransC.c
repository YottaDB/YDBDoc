...
ydb_buffer_t Crab, Delta, Horse, /* Buffers for names of global variables used in this application */
	sub1[1], /* sub1 & sub2 are used to pass subscripts; just 1 per node in this application */
	sub2[1],
	valb; /* Buffer used to pass values between application and YottaDB */
...
int main()
{
	...
	sub1[0].len_alloc = YDB_MAXINT_BUFSIZ; /*	All parameters & values are integers with pre-allocated buffers */
	sub2[0].len_alloc = YDB_MAXINT_BUFSIZ;
	valb.len_alloc = YDB_MAXINT_BUFSIZ;
	sub1[0].buf_addr = &currtime[0]; /* sub1 is used only for the current time, since it is used repeatedly */
