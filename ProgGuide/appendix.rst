...
The :code:`ydbsh` executable allows you to create `shebang <https://en.wikipedia.org/wiki/Shebang_(Unix)>`_ style scripts with M code. A file with an alphanumeric name (i.e., just letters and digits; unlike M routines, a leading underscore is not allowed) and an extension of :code:`.m`, has execute permissions, and whose first line is:
...
Since the script is an M routine, it must follow the normal syntax of M code, e.g., whitespace at the beginning of lines other than lines with labels. The name of the routine, `$TEXT(+0) <functions.html#text>`_ is the filename of the shebang script without the :code:`.m` extension.
...
.. code-block:: none
   ...
   #!/usr/local/bin/ydbsh
	   ...
	   . write $reverse(line),!
   $ /tmp/ydbsh.m ABC DEF
   /tmp/ydbshUvgzJC(/tmp) /extra/usr/local/lib/yottadb/r202/libyottadbutil.so
   /tmp/ydbsh.m ABC DEF
