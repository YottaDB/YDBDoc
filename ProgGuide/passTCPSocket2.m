...
; Example M program to demonstrate how to pass a connected TCP socket to a
; child process through a local socket.

; The `passTCPSocket2` routine runs as the parent process. It will spawn and
; coordinate socket operations between two child processes. One child process
; will act as a basic TCP client, while the other will act as a worker. The parent
; process functions as a server.

; The `passTCPSocket2` routine will first spawn the TCP client process. Then it
; will open a TCP socket to listen for TCP requests from the client process.
; The original `passTCPSocket` passed this LISTENING socket to the the worker job;
; but this program waits for a connection from the client, and passes the CONNECTED
; socket (NOT the LISTENING socket) to the worker.
...
passTCPSocket2 ;
	...
	zsystem "rm client.out worker.out"
	...
	job worker:(output="worker.out")         ; Spawn job to receive the detached TCP socket
	...
	use "tcpSocket":(chset="M")          ; Set the new socket as the current I/O device
	write /listen(1)                     ; Set listen queue size to 1
	write /wait(10)                      ; Wait for client to connect for up to 10 seconds
	set handle=$piece($key,"|",2)        ; Extract socket handle from socket state information
	use "tcpSocket":(detach=handle)      ; Detach the listening socket
	...
	; Wait for child and worker to communicate
	...
	zshow "d"
	write "----------",!
	write "Contents of `client.out` and `worker.out` for results:",!
	write !
	write "client.out",!
	write "----------",!
	...
	write "worker.out",!
	write "----------",!
	zsystem "cat worker.out"
	quit

; Simple routine that acts as a rudimentary TCP worker. Spawned as a child of the `passTCPSocket` routine.
worker   ;
	new handle,input
	write "Worker process pid = ",$job,!
	new $etrap set $etrap="goto handleError"
