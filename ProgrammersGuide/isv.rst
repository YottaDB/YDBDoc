...
* If the result is a positive number greater than or equal to 86400, add one to the first piece and subtract 86400 from the second piece.
...
If the environment variable :code:`ydb_routines` is not set when the :code:`yottadb` process starts, or if it is set to the empty string (:code:`""`), than $ZROUTINES will include the following.

* In M mode $ZROUTINES will include:

  * all shared libraries in :code:`$ydb_dist/plugin/o`
  * :code:`$ydb_dist/libyottadbutil.so` if it exists, otherwise :code:`$ydb_dist`

* In UTF-8 mode $ZROUTINES will include:

  * all shared libraries in :code:`$ydb_dist/plugin/o/utf8`
  * :code:`$ydb_dist/utf8/libyottadbutil.so` if it exists; otherwise :code:`$ydb_dist/utf8` if it exists; otherwise :code:`yottadb` exits with an error
