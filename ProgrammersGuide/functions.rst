...
+------------------+------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| "STP_GCOL_NOSORT"| none             | Returns 0 if garbage collections use the sorted approach (i.e. `VIEW "STP_GCOL_NOSORT":0 <../ProgrammersGuide/commands.html#view-stpgcolnosort>`_), the default,    |
|                  |                  | and 1 if they use the unsorted approach (i.e. `VIEW "STP_GCOL_NOSORT":1 <../ProgrammersGuide/commands.html#view-stpgcolnosort>`_).                                  |
+------------------+------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
* $ZGETJPI() retrieves process information. It retrieves the time measurements (CPUTIM, CSTIM, CUTIME, STIME, and UTIME) as well as the command line (CMDLINE) for requested process. If the information is not available for the specified process, the function returns -1 if the output type is an integer (CPUTIM, CSTIME, CUTIME, STIME, and UTIME) or an empty string if output type is a string (CMDLINE). If the process ID specified is 0, the current process ($JOB) time measurements are returned. The "child" process time includes `ZSYSTEM <commands.html#zsystem>`_ and `PIPE <ioproc.html#using-pipe-devices>`_ device sub-processes (only after the PIPE CLOSEs), but excludes processes created by `JOB <commands.html#job>`_ commands.
